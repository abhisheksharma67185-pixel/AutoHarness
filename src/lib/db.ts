...
const isPostgres = typeof process.env.DATABASE_URL === 'string' && 
...
if (isPostgres) {
  pool = new Pool({
    ...
    ssl: { rejectUnauthorized: false } // Required for Supabase/Neon connection
  });
  console.log('Using Supabase/Postgres database connection pool');
} else {
  ...
  // Re-initialize SQLite database schema
  sqliteDb.exec(`
...
export const db = {
  ...
  async run(text: string, params: any[] = []): Promise<{ lastInsertRowid: any }> {
    if (isPostgres && pool) {
      ...
      const id = insertedRow ? (insertedRow.id || insertedRow.lastinsertrowid) : null;
      return { lastInsertRowid: id };
    } else if (sqliteDb) {
      const res = sqliteDb.prepare(text).run(...params);
