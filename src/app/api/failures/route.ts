...
export async function GET(req: NextRequest) {
  try {
    ...
    const response = await fetch(`http://localhost:8001/api/v1/runs/failure-modes?run_id=${encodeURIComponent(run_id)}`, { cache: 'no-store' });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const response = await fetch('http://localhost:8001/api/v1/runs/failure-modes/update', {
      method: 'POST',
